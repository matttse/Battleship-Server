...
public class BSGame extends JFrame implements ActionListener, MouseListener {
	...
	private static final String revision = "2.0.1";
	...
	public BSGame(int type) {
	    ...
	    p1 = new Player1(BSGame.GAME_PLAYER1);  // Insert your player here!


	    p2 = new Player2(BSGame.GAME_PLAYER2);  // Insert your player here!
	    ...
	    displayButton = new JButton("Clear Debug Text");
	    displayButton.addActionListener(this);
	    displayButton.setActionCommand("clrDbg");
	    b3 = new JButton("Reset Game");
	    b3.setActionCommand("p1");
	    b3.addActionListener(this);

	    b4 = new JButton("Toggle Debug");
	...
	public void paint(Graphics g){
		...
		if (boardsAreVisible) {
			...
			drawBoard(g, topRightBoard, BSGame.BOARD_P1MOVES, 400, 0);
			drawBoard(g, bottomLeftBoard, BSGame.BOARD_P2SHIPS, 0, 400);
			drawBoard(g, bottomRightBoard, BSGame.BOARD_P2MOVES, 400, 400);
		}
		...
		case GAME_PLAYER1:
		case GAME_PLAYER2:
			g.drawString(p1.getName() + " Ships", BSGame.X_OFFSET, 190);
			g.drawString(p1.getName() + " Moves", BSGame.X_OFFSET+400, 190);
			g.drawString(p2.getName() + " Ships", BSGame.X_OFFSET, 590);
			g.drawString(p2.getName() + " Moves", BSGame.X_OFFSET+400, 590);
			break;
	...
	private void drawMove(int loc, int row, int col, Color color) {
		...
		g.fillOval(((loc%2)*400)+X_OFFSET+10+(SQUARE_SIZE*col),
				myLoc + 8 +(SQUARE_SIZE * row), 12, 12);
		g.setColor(oldColor);
	}
	...
	public boolean makeMove(int shipBd, int moveBd, int row, int col) {
		...
		if (GAME_LAST_PLAYER == moveBd){
			...
			if ( (p2.startingTurn == true) && (moveBd == BSGame.BOARD_P1MOVES)){
				...
				//dbg.addText("someone is cheating!");
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (arg0.getActionCommand().equals("clrDbg")) {
			dbg.clearText();
			repaint();
